...
echo [7m Hello. This is the RemoveXlsxImages CMD script. It will remove most images from your XLSX files.[0m
echo [7m Please do not hesitate to close this window if you think that something is going wrong.[0m
echo [7m Drag and drop your XLSX file on this file icon to strip it from most images.[0m
...
if errorlevel 1 goto sevenzip_error
ren "%newname%.zip" "%~nx1_%newname%.xlsx"

if not exist "%~nx1_%newname%.xlsx" goto error_file_not_created

move "%~nx1_%newname%.xlsx" "%~dp1"
...
:error_bad_filename
echo [91m Error: the input file does not have a XLSX/xlsx extension. [0m
goto exit
